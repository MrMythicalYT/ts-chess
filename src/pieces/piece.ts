...
function checkKnight(piece: Piece, array: Location[]): void {
  ...
  array.push(
    ...(
      ...
      (l) =>
        l.every((i) => i < 8 && i >= 0) &&
        piece.board.toArray()[l[0]]?.[l[1]]?.side !== piece.side
    )
  );
}
...
function checkVerticalHorizantalDiagonal(
  piece: Piece,
  array: Location[]
): void {
...
function checkVerticalHorizantalDiagonal1(
  piece: Piece,
  array: Location[]
): void {
  let x = piece.location[0];
...
abstract class Piece {
  ...
  get location(): Location {
    ...
    if (typeof x === "undefined")
      throw new Error("Cannot find location x on board");
    if (typeof y === "undefined")
      throw new Error("Cannot find location y on board");
  ...
  get attacked(): boolean {
    return this.board
      ...
      .some((p) =>
        p?.unlegalizedMoves.some(
          (a) => a[0] === this.location[0] && a[1] === this.location[1]
        )
      );
  }
    ...
    return (
      this.unlegalizedMoves?.filter((move) => {
        return !this.board.clone()
          .move(this, this.location, move)
          .inCheck({ side: this.side });
      }) ?? []
    );
  }
  move(location: Location): this {
    this.board.move(this, this.location, location)
    return this;
  }
  side: Side;
  board: Board;
